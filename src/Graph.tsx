...
import { EdgeConfig, Graph as Graph2, NodeConfig, registerEdge, registerNode, Node, Edge } from "@antv/g6";
...
export default function Graph(props: GraphContainerProps) {
    ...
    useEffect(() => {
        function cb(evt: any) {
            ...
            const uid = item?.getModel().data;
            if (item instanceof Node && nodes && uid && props.nodes?.items) {
                const idx = nodes.findIndex(d => d.data === uid);
                if (idx !== -1) {
                    const obj = props.nodes?.items[idx];

                    const action = props.dbclickNode?.get(obj);
            ...
            if (item instanceof Edge && edges && uid && props.edges?.items) {
                const idx = edges.findIndex(d => d.data === uid);
                if (idx !== -1) {
                    const obj = props.edges?.items[idx];

                    const action = props.dbclickEdge?.get(obj);
                    if (action && action.canExecute && !action.isExecuting) {
                        action.execute();
                    }
                }
